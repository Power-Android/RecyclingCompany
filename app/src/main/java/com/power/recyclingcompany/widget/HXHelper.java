package com.power.recyclingcompany.widget;
...
public class HXHelper {
    ...
    private EaseUser getUserInfo(String username){
        ...
        if(username.equals(EMClient.getInstance().getCurrentUser())){
            ...
            user.setAvatar(SPUtils.getInstance().getString(Constant.keys.FACE));
            user.setNick(SPUtils.getInstance().getString(Constant.keys.USERNAME));
            return user;
        }else {//这里写死了  以后从数据库获取 如果数据库没有从服务器获取
            user = new EaseUser(username);
            user.setAvatar("http://img5.duitang.com/uploads/item/201508/30/20150830132007_TjANX.thumb.224_0.jpeg");
            user.setNick("Power回收司机");
        }
    	...
    	messageListener = new EMMessageListener() {
			...
			public void onMessageReceived(List<EMMessage> messages) {
			    for (EMMessage message : messages) {
                    ...
                    //接收并处理扩展消息
                    String userName=message.getStringAttribute(Constant.keys.USERNAME,"");
                    String userPic=message.getStringAttribute(Constant.keys.FACE,"");
                    String hxIdFrom=message.getFrom();
                    LogUtils.e("helper接收到的用户名："+userName+"helper头像："+userPic);
